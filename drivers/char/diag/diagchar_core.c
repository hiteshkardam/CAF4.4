...
static int diagchar_setup_cdev(dev_t devno)
{
	...
	if (err)
		pr_debug("dev_attr_diag_reg_table registration failed !\n\n");
	err = device_create_file(driver->diag_dev, &dev_attr_diag7k_debug_mask);
	if (err)
		pr_debug("dev_attr_diag7k_debug_mask registration failed !\n\n");
	err = device_create_file(driver->diag_dev, &dev_attr_diag9k_debug_mask);
	if (err)
		pr_debug("dev_attr_diag9k_debug_mask registration failed !\n\n");
/*++ 2015/10/26, USB Team, PCN00033 ++*/
	err = device_create_file(driver->diag_dev, &dev_attr_diag_rb);
	if (err)
		pr_debug("dev_attr_diag_rb registration failed !\n\n");
/*-- 2015/10/26, USB Team, PCN00033 --*/
...
//	pr_debug("diagfwd initializing, kernel_flag=[0x%x] ..\n", kernel_flag);
